...
public class FumedemeExpoWidgetModule: Module {
      public func definition() -> ModuleDefinition {
        ...
        Function("setItem") { (value: String, key: String, appGroup: String) -> Void in
            NSLog("[FumedemeExpoWidgetModule] setItem key=%@ value=%@", key, value)
            if let userDefaults = UserDefaults(suiteName: appGroup){
                userDefaults.set(value, forKey: key )
            } else {
                NSLog("[FumedemeExpoWidgetModule] setItem could not open suite %@", appGroup)
            }
        }
